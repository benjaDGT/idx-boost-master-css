  ...
  $.loadMapNeighrborhoods = function(){
        ...
        $.each( dgtCredential.item_neighboardhood, function( index, community ) {
          ...
          if(typeof community.geometry != "undefined" && community.geometry != null){
            tglat.push(community['lat']);
            tglng.push(community['lng']);
            tglngsu=tglngsu+parseFloat(community['lng']);
            tglatsu=tglatsu+parseFloat(community['lat']);            
            totitem=totitem+1;
            
            locpolyn = new google.maps.LatLng(parseFloat(community.geometry_lat), parseFloat(community.geometry_lng));
        ...
        //tgmap.setCenter(new google.maps.LatLng( tglatsu/totitem,tglngsu/totitem));

  /*  $.ajax({
      url: dgtCredential.ajaxUrl,
      type: "POST",
      data: {action: 'dgt_load_neighborhoods', only_special: dgtCredential.only_special, counter: dgtCredential.counter},
      dataType: "json",
