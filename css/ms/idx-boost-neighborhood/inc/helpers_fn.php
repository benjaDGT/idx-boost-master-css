...
if (!function_exists('neighborhood_admin_register_assets')) {
    function neighborhood_admin_register_assets()
    {   $flex_idx_info_neigh         = flex_idx_get_info_for_neighboardhood();
        wp_enqueue_style( 'idx-neighborhood-admin-css', NEIGHBORHOOD_IDX_URI . 'css/admin_neighborhood.css', array(), NEIGHBORHOOD_IDX_VERSION );
        wp_enqueue_script( 'google-maps-js', 'https://maps.googleapis.com/maps/api/js?v=3&libraries=drawing&key=' . $flex_idx_info_neigh["agent"]["google_maps_api_key"], array('jquery'), NEIGHBORHOOD_IDX_VERSION, true );
        // wp_enqueue_script( 'markerwithlabel-packed-js', NEIGHBORHOOD_IDX_URI . 'js/markerwithlabel_packed.js', array('jquery'), NEIGHBORHOOD_IDX_VERSION);
        wp_enqueue_script( 'infobox-packed-js', NEIGHBORHOOD_IDX_URI . 'js/infobox_packed.js', array('jquery'), NEIGHBORHOOD_IDX_VERSION, true );
        wp_enqueue_script( 'gmap-draw-js', NEIGHBORHOOD_IDX_URI . 'js/jquery.gmap-draw.js', array('jquery'), NEIGHBORHOOD_IDX_VERSION, true );
        wp_enqueue_script( 'GDouglasPeuker-js', NEIGHBORHOOD_IDX_URI . 'js/GDouglasPeuker.js', array('jquery'), NEIGHBORHOOD_IDX_VERSION, true );
        wp_enqueue_script( 'idx-neighborhood-admin-js', NEIGHBORHOOD_IDX_URI . 'js/admin_neighborhood.js', array('jquery'), NEIGHBORHOOD_IDX_VERSION, true );
        wp_localize_script('idx-neighborhood-admin-js', 'dgtCredential', array('ajaxUrl' => admin_url('admin-ajax.php')));
    }
}

if(!function_exists('dgt_get_geocode_fn')) {
    function dgt_get_geocode_fn() {
        $flex_idx_info_neigh         = flex_idx_get_info_for_neighboardhood();
        $address='';
...
if (!function_exists('idxboost_convert_image_map')) {

function idxboost_convert_image_map($mdata) {
        global $wpdb;
        $image_map_kml='';
        $flex_idx_info_neigh         = flex_idx_get_info_for_neighboardhood();
        ...
        $lat = $mdata['lat'];
        $lng = $mdata['lng'];
        $zoom = ($mdata['zoom'] > 0) ? $mdata['zoom'] : 14;
        $zoom= $zoom-2;
        if (!empty($mdata['geometry'])):
            $geocode = $mdata['geometry'];
            $geometry=$mdata;
            //parse_str($geocode, $geometry);
            $row['geometry'] = $geometry['geometry'];
            $geometry = str_replace(array('POLYGON ((', '))'), '', $geometry['geometry']);
            $geometry = explode(', ', $geometry);

            $fillcolor = str_replace('#', '0x', '#0072ac') . '1A';
            $strokeColor = str_replace('#', '0x', '#0072ac') . 'FF';
            $center_map = 'center=' . $lat . ',' . $lng;
            $zoom_map = 'zoom=' . $zoom;
            $type_map = 'maptype=roadmap&size=500x250&scale=2';
            $image_map = 'path=fillcolor:' . $fillcolor . '%7Cweight:1%7Ccolor:' . $strokeColor;

            $style_map = '';
            $style_map .= 'style=feature:landscape';
            $style_map .= '%7Csaturation:' . '-100';
            $style_map .= '%7Clightness:' . '65';
            $style_map .= '%7Cvisibility:' . 'on';

            $style_map .= '&style=feature:' . 'poi';
            $style_map .= '%7Csaturation:' . '-100';
            $style_map .= '%7Clightness:' . '51';
            $style_map .= '%7Cvisibility:' . 'simplified';

            $style_map .= '&style=feature:' . 'road.highway';
            $style_map .= '%7Csaturation:' . '-100';
            $style_map .= '%7Cvisibility:' . 'simplified';

            $style_map .= '&style=feature:' . 'road.arterial';
            $style_map .= '%7Csaturation:' . '-100';
            $style_map .= '%7Clightness:' . '30';
            $style_map .= '%7Cvisibility:' . 'on';

            $style_map .= '&style=feature:' . "road.local";
            $style_map .= '%7Csaturation:' . '-100';
            $style_map .= '%7Clightness:' . '40';
            $style_map .= '%7Cvisibility:' . 'on';

            $style_map .= '&style=feature:' . "transit";
            $style_map .= '%7Csaturation:' . '-100';
            $style_map .= '%7Cvisibility:' . 'simplified';

            $style_map .= '&style=feature:' . "administrative.province";
            $style_map .= '%7Cvisibility:' . 'off';

            $style_map .= '&style=feature:' . "water";
            $style_map .= '%7Celement:' . 'labels';
            $style_map .= '%7Cvisibility:' . 'on';
            $style_map .= '%7Csaturation:' . '-100';
            $style_map .= '%7Clightness:' . '-25';

            $style_map .= '&style=feature:' . "water";
            $style_map .= '%7Celement:' . 'geometry';
            $style_map .= '%7Chue:0x211f20';
            $style_map .= '%7Csaturation:' . '-97';
            $style_map .= '%7Clightness:' . '-25';

            $key_map = 'key='.$flex_idx_info_neigh["agent"]["google_maps_api_key"];

            foreach ($geometry as $item) {
                $data = explode(' ', $item);
                $image_map .= '%7C' . round($data[1], 8) . ',' . round($data[0], 8);
            }
            $image_map_kml = 'https://maps.googleapis.com/maps/api/staticmap?' . $center_map . '&' . $zoom_map . '&' . $type_map . '&' . $image_map . '&' . $style_map . '&' . $key_map;
        endif;

        return $image_map_kml;
    }
}


function dgt_load_neighborhoods_fn( ) {
    $only_special =  $_POST['only_special'];
    $counter =  $_POST['counter'];
    $query_arr = array(
      'post_type' => NEIGHBORHOOD_IDX_NAME,
      'orderby' => 'title',
      'order' => 'ASC',
    ...
    if($only_special == true) {
      $query_arr['meta_query'] = array(
        array(
          'key'     => DGTIDX_PREFIX . '_extra_special',
          'value'   => 1,
          ),
        );
    }
...
class IdxboostPagTemplateNeighborhoods {
    ...
    public static function get_instance() {
        ...
        if ( null == self::$instance ) {
            self::$instance = new IdxboostPagTemplateNeighborhoods();
        } 
    ...
    private function __construct() {
        ...
        $this->templates = array(
            'flex-page-neighborhood.php' => 'TG Neighborhoods Collection',
            'flex-page-neighborhood-detail-silo.php' => 'TG Neighborhoods Details',
            'idx-page-neighborhood-news.php' => 'TG Neighborhoods Details News',
        );
    } 
    ...
    public function view_project_template( $template ) {
        ...
        $file = NEIGHBORHOOD_IDX_PATH.'views/template/'. get_post_meta(  $post->ID, '_wp_page_template', true );
...
add_action( 'plugins_loaded', array( 'IdxboostPagTemplateNeighborhoods', 'get_instance' ) );